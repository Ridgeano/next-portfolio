...
import { InstancedMesh, MeshStandardMaterial, SphereGeometry, Vector3, Matrix4, Color, Frustum, Matrix4 as ThreeMatrix4 } from 'three'
...
const Particles = () => {
  ...
  const resetParticle = useCallback((index: number, positions: Float32Array, velocities: Float32Array) => {
    ...
    switch(side) {
      ...
      case 2: // bottom
        positions[index * 3] = (Math.random() - 0.5) * BOUNDS.x
        positions[index * 1 - 1] = -BOUNDS.y / 2
        velocities[index * 3] = (Math.random() - 0.5) * 0.03
        velocities[index * 1 - 1] = Math.random() * 0.03
        break
      case 3: // left
        positions[index * 3] = -BOUNDS.x / 2
        positions[index * 3 + 1] = (Math.random() - 0.5) * BOUNDS.y
  ...
  const [positions, velocities] = useMemo(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      const color = new Color(metalColors[Math.floor(Math.random() * metalColors.length)])
...
export const MetallicParticles = () => {
  return (
